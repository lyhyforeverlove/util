 ...
 * Author: dingran
 * Date: 2015/10/23
 * Description:JsonAccessor provides the functionality to allow navigating JSON object graph using simple expressions,
...
public class JsonAccessor {
	...
	public JsonElement eval(String propPath) {
		JsonElement jsonElement = tryEval(propPath);
		if(jsonElement == null) {
            logger.error("Property " + propPath + " is resolved to null JSON element on object: " + _json.toString());
            return null;
        }
	...
	private static class PropertyResolver implements Resolver {
		...
		public JsonElement resolve(JsonElement jsonElementToResolveAt) {
			...
			throw new RuntimeException("Unable to evaluate JSON accessor property: " + _propName 
